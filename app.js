let cardData = [
  {
    ...
    imgLink: "./images/suliman1-post.PNG",
...
const previewImage = document.getElementById("img_url");
const previewVideo = document.getElementById("vedio_url");
...
fileInput.addEventListener("change", (event) => {
  if (event.target.files.length > 0) {
    if ((event.target.files[0].type = "image/jpeg")) {
      ...
      previewVideo.src = URL.createObjectURL(event.target.files[0]);
    }
